...
gaConfig.mutationProbability=0.1;
...
generation=1000;
% optimizing coverage
while sum(optimizor.cover==1)==0 && optimizor.count<20
	for agent_number = 1:number_of_spicies
	    % population(agent_number)= InitializePopulation(map, gaConfig);
	    Evaluating(population(agent_number),map,gaConfig);
	end
...
while  sum(abs(diff(optimizor.record_dis_working(max(end-200,1):end))))~=0 && optimizor.count<2000
